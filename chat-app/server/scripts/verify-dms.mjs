...
try {
  ...
  assert.ok(cRooms.includes('general'), 'public rooms stay visible to everyone');
  ...
  assert.ok(aRooms.includes('general'), 'participant still sees public rooms');
  assert.equal((await get('/api/rooms', a.token)).rooms.find((r) => r.name === 'general').type, 'public');
  ...
  const backToGeneral = event(a.s, 'room_joined', d => d.room === 'general');
  a.s.emit('join_room', { room: 'general' });
  await backToGeneral;
